...
//ConVar r_buildingmapforworld( "r_buildingmapforworld", "0" ); //IV Note: Only Works with new Source Engine. In Older Version this logic dosen't exsists

#include "IVWaterCheap_vs20.inc"
//#include "WaterCheap_ps20.inc"
#include "IVWaterCheap_ps20b.inc"
#include "IVWater_vs20.inc"
//#include "water_ps20.inc"
#include "IVwater_ps20b.inc"
#include "IVWater_ps30.inc"
#include "IVWater_vs30.inc"
...
BEGIN_VS_SHADER( IVWater_DX90, 
			  "Help for Water Ported from CS:GO Engine" )
	...
	inline void DrawReflectionRefraction( IMaterialVar **params, IShaderShadow* pShaderShadow,
		IShaderDynamicAPI* pShaderAPI, bool HasFlashlight, bool bReflection, bool bRefraction ) 
	{
		Vector4D Scroll1;
		...
		SHADOW_STATE
		{
			...
			{
				DECLARE_STATIC_VERTEX_SHADER(IVWater_vs30);
				...
				SET_STATIC_VERTEX_SHADER(IVWater_vs30);
			}
			else
			{
				DECLARE_STATIC_VERTEX_SHADER(IVWater_vs20);
				SET_STATIC_VERTEX_SHADER_COMBO(MULTITEXTURE, bHasMultiTexture);
				SET_STATIC_VERTEX_SHADER_COMBO(BASETEXTURE, bHasBaseTexture);
				SET_STATIC_VERTEX_SHADER_COMBO(FLASHLIGHT, hasFlashlight);
				SET_STATIC_VERTEX_SHADER_COMBO(LIGHTMAPWATERFOG, bLightmapWaterFog);
				SET_STATIC_VERTEX_SHADER_COMBO(FLOWMAP, bHasFlowmap);
				SET_STATIC_VERTEX_SHADER(IVWater_vs20);
			}
			...
			if (g_pHardwareConfig->SupportsShaderModel_3_0())
			{
				DECLARE_STATIC_PIXEL_SHADER(IVWater_ps30);
			...
			{
				DECLARE_STATIC_PIXEL_SHADER( IVWater_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO( REFLECT,  bReflection );
				SET_STATIC_PIXEL_SHADER_COMBO( REFRACT,  bRefraction );
				SET_STATIC_PIXEL_SHADER_COMBO( ABOVEWATER,  params[ABOVEWATER]->GetIntValue() );
				SET_STATIC_PIXEL_SHADER_COMBO( MULTITEXTURE, bHasMultiTexture );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FORCEFRESNEL, bForceFresnel );
				SET_STATIC_PIXEL_SHADER_COMBO( SIMPLEOVERLAY, bHasSimpleOverlay );
				SET_STATIC_PIXEL_SHADER( IVWater_ps20b );
			}
			/*else
			{
...
//				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
		...
		DYNAMIC_STATE
		{
			...
			if (g_pHardwareConfig->SupportsShaderModel_3_0())
			{
				DECLARE_DYNAMIC_VERTEX_SHADER(IVWater_vs30);
				SET_DYNAMIC_VERTEX_SHADER(IVWater_vs30);
			}
			else
			{
				DECLARE_DYNAMIC_VERTEX_SHADER(IVWater_vs20);
				SET_DYNAMIC_VERTEX_SHADER(IVWater_vs20);
			}
...
/*#ifdef _PS3
				...
				if (pFlashlightDepthTexture && g_pConfig->ShadowDepthTexture() && flashlightState.m_bEnableShadows)
				{
					...
					bFlashlightShadows = bFlashlightShadows && flashlightState.m_bEnableShadows;
				}
				...
				lightPos[3] = flashlightState.m_FarZ;
				pShaderAPI->SetPixelShaderConstant(16, lightPos, 1);

				float tint_value[3] = { params[FLASHLIGHTTINT]->GetFloatValue(), params[FLASHLIGHTTINT]->GetFloatValue(), params[FLASHLIGHTTINT]->GetFloatValue() };

				pShaderAPI->SetPixelShaderConstant(10, tint_value);
			}
		                        ...
		                        IShaderDynamicAPI* pShaderAPI, bool bBlend, bool bRefraction )
	{
		...
		SHADOW_STATE
		{
			...
			DECLARE_STATIC_VERTEX_SHADER( IVWaterCheap_vs20 );
			SET_STATIC_VERTEX_SHADER_COMBO( BLEND,  bBlend && bRefraction );
			SET_STATIC_VERTEX_SHADER( IVWaterCheap_vs20 );

			if( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			{
				DECLARE_STATIC_PIXEL_SHADER( IVWaterCheap_ps20b );
				...
				SET_STATIC_PIXEL_SHADER_COMBO( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO( FLOW_DEBUG, clamp( params[ FLOW_DEBUG ]->GetIntValue(), 0, 2 ) );
				SET_STATIC_PIXEL_SHADER( IVWaterCheap_ps20b );
			}
			/*else
			{
		...
		DYNAMIC_STATE
		{
			...
			DECLARE_DYNAMIC_VERTEX_SHADER( IVWaterCheap_vs20 );
			SET_DYNAMIC_VERTEX_SHADER( IVWaterCheap_vs20 );
	...
	SHADER_DRAW
	{
		...
		if ( ( bReflection || bRefraction || bEnvMap ) && !UsingEditor( params ) && !bForceCheap )
		{
			...
			bool bFlashlight = UsingFlashlight(params);

			DrawReflectionRefraction(params, pShaderShadow, pShaderAPI, bFlashlight, bReflection, bRefraction);
...
//-----------------------------------------------------------------------------
// This allows us to use a block labelled 'IVWater_DX9_HDR' in the water materials
//-----------------------------------------------------------------------------
BEGIN_INHERITED_SHADER( IVWater_DX9_HDR, IVWater_DX90,
			  "Help for IVWater_DX9_HDR" )
